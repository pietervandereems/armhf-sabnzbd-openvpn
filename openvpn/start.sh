...
# add sabnzb credentials from env vars
#echo $SABNZB_WEB_USER > /config/sabnzb-credentials.txt
#echo $SABNZB_WEB_PASSWORD >> /config/sabnzb-credentials.txt

# Persist sabnzb settings for use by sabnzbd
#dockerize -template /etc/sabnzb/environment-variables.tmpl:/etc/sabnzb/environment-variables.sh /bin/true

SABNZB_CONTROL_OPTS="--script-security 2 --up /etc/sabnzbd/start.sh --down /etc/sabnzbd/stop.sh"
...
exec openvpn $SABNZB_CONTROL_OPTS $OPENVPN_OPTS --config "$OPENVPN_CONFIG"
